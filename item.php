    ...
    <body style="background: url(<?php echo 'HolidayFriendAdmin/'.$bg_url; ?>)no-repeat fixed center center; min-height:969px;" class="pb-6 bgimg">
        ...
        <div class="main-content" id="panel" >
            ...
            <div class="mt-3">
                ...
                <?php 
    $moduleName = "";
        $sql="SELECT * FROM `tbl_user_module_map` WHERE `user_id` = $user_id and `umod_id` = 13";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            while($row = mysqli_fetch_array($result)) {
        ...
        if ($moduleName==""){
            $sql="SELECT * FROM `tbl_util_modules` WHERE `umod_id` = 13";
            $result = $conn->query($sql);
            if ($result && $result->num_rows > 0) {
                while($row = mysqli_fetch_array($result)) {
                ...
                <div class="row">
                    <div class="col-xl-4 col-md-4 col-sm-4 div-shadow-mobile mt-2">
                        ...
                        $sql="SELECT a.*,b.link_url FROM `tbl_custom_entity` as a INNER JOIN tbl_custom_entity_type as b on a.custety_id=b.custety_id WHERE `hotel_id` = $hotel_id AND isactive=1 Order By position_order";
                        ...
                        if ($result && $result->num_rows > 0) {
                            while($row = mysqli_fetch_array($result)) {
                                ...
                                $sql_inner="SELECT img_url FROM `tbl_custom_entity_gallery` where custe_id=$id";
                    ...
                    <?php }else{?>
                    <div class="mr-3 ml-3 mt-4 pointer bg-affect-div" onclick="show_details('<?php echo $link_url; ?>',<?php echo $id; ?>)">
                        <img src="HolidayFriendAdmin/<?php echo $img_url; ?>" alt="" class="card-img-top">
                    </div>
                    <?php } ?>
                    <div class="bg-white-opacity mobile-margin-information mr-3 ml-3 pr-3 pl-3 pt-3 pb-1 hover-affect-div pointer bg-affect-div div-shadow-mobile" onclick="show_details('<?php echo $link_url; ?>',<?php echo $id; ?>)">
                        ...
                        <p><?php echo substr($row['description'],0,200); ?> [more]</p>
                        <?php } ?>
                    </div>
                    <?php
                                    $i++;
                ...
                <div class="col-xl-4 col-md-4 col-sm-4">
                    ...
                    <?php } ?>
                    <div class="bg-white-opacity mobile-margin-information mr-3 ml-3 pr-3 pl-3 pt-3 pb-1 hover-affect-div pointer bg-affect-div " onclick="show_details('<?php echo $link_url; ?>',<?php echo $id; ?>)">
                        <div class="div-title-background-1">
                            <h2 class="heading-title-background"><?php echo $row['title'] ?></h2>
                        </div>
                        <hr class="m-0 mb-3">

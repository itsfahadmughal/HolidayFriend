                    ...
                    <div class="col-xl-12 col-md-12 col-sm-12 bg-white-opacity align-text-center p-3 scroll-bar-x-smallscreen div-shadow-mobile bgfull-affect-div">
                        <table class="w-100">
                            <tbody>
                                <?php
                                if($cat_id>0){
                                    $sql="SELECT a.*,c.measuring_unit,b.tag_title FROM `tbl_skiing_slope` as a INNER JOIN tbl_util_measuring_units as c ON a.munit_id=c.munit_id INNER JOIN tbl_util_tags as b on a.utag_id=b.utag_id WHERE a.hotel_id=$hotel_id AND a.isactive=1 AND a.area_id=$cat_id ORDER BY a.area_id";
                                }else{
                                    $sql="SELECT a.*,c.measuring_unit,b.tag_title FROM `tbl_skiing_slope` as a INNER JOIN tbl_util_measuring_units as c ON a.munit_id=c.munit_id INNER JOIN tbl_util_tags as b on a.utag_id=b.utag_id WHERE hotel_id=$hotel_id AND isactive=1 ORDER BY area_id";
                                }
                                ...
                                if ($result && $result->num_rows > 0) {
                                    while($row = mysqli_fetch_array($result)) {
                                        if($i==1){
                                            $j=2;
                                            $skisl_id=$row['skisl_id'];
                                ?>

                                <h1 class=" page-title-text" style="text-align:left;">Slopes<?php if($cat_id>0){ echo ' - '.$cat_name;} ?></h1>

                                <?php
                                        }
                                        if($row['skisl_id']==$skisl_id){
                                ...
                                <?php if($row['skisl_id']!=$skisl_id && $j==1){

                                }else{ ?>
                                <tr>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">Name</th>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">State</th>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">Tag</th>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">Opening Hours</th>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">Length</th>
                                    <th class="pr-5 pl-2 pt-3 pb-3 ">Altitude</th>
                                    <th class="pr-2 pl-2 pt-3 pb-3">Nr.</th>
                                </tr>
                                <?php 
                                    $j=1;} ?>
                                ...
                                <tr <?php if($k%2!=0){?> style="background-color:#e9e9e9" <?php } ?>>
                                    ...
                                    <td class="pr-5 pl-2 pt-3 pb-3"><?php echo $row['tag_title'] ?></td>
                                    ...
                                    <td class="pr-5 pl-2 pt-3 pb-3 "><?php echo $row['length'].' '.$row['measuring_unit'] ?></td>
                                    <td class="pr-5 pl-2 pt-3 pb-3 "><?php echo $row['alt_difference'].' '.$row['measuring_unit'] ?></td>
                                    <td class="pr-2 pl-2 pt-3 pb-3 "><?php echo $row['id_num'] ?></td>
                                </tr>

...
if(isset($_GET['id'])){
    ...
    $sql="SELECT * FROM `tbl_items` Where item_id=$item_id AND `isactive` = 1";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
        while($row = mysqli_fetch_array($result)) {
            $title=$row['title'];
    ...
    //Images
    $sql3="SELECT `img_url` FROM `tbl_items_gallery` WHERE `item_id` = $item_id ";
...
<html>
    ...
    <head>
        ...
        <title>Custom Entity Detail</title>
        <!-- Favicon -->
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-xl-5 col-md-5 col-sm-5">
                        <h3 class="textstyle back-button" onclick="window.location = 'customentity'"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</h3>
                    </div>
        ...
        if($length>0){
            ...
            for ($x = 0; $x < $length; $x++) {
                    ...
                    <div class ="col-xl-7 col-md-7 col-sm-7  pr-3 pl-3 pb-3 mobile_padding_none">
                        ...
                        <?php if($description != ""){ ?>
                        <div class="row m-0 bg-white-opacity  pr-3 pl-3 pb-3 bgfull-affect-div">

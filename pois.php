...
<html>
    ...
    <head>
        ...
        <title>Hotel - In the Surround</title>
        <!-- Favicon -->
        ...
        <div class="main-content" id="panel" >
            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row pt-3">
                    <?php 
    $moduleName = "";
        $sql="SELECT * FROM `tbl_user_module_map` WHERE `user_id` = $user_id and `umod_id` = 11";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            while($row = mysqli_fetch_array($result)) {
        ...
        if ($moduleName==""){
            $sql="SELECT * FROM `tbl_util_modules` WHERE `umod_id` = 11";
            $result = $conn->query($sql);
            if ($result && $result->num_rows > 0) {
                while($row = mysqli_fetch_array($result)) {
                ...
                <div class="row">
                    ...
                    <?php
                    $sql="SELECT DISTINCT a.* FROM `tbl_in_surround_cat` as a INNER JOIN tbl_in_surround_subcat as b ON a.insc_id = b.insc_id INNER JOIN tbl_in_surround as c on b.insb_id=c.insb_id WHERE c.hotel_id = $hotel_id and c.isactive = 1 ORDER BY c.position_order";
                    $result = $conn->query($sql);       
                    if ($result && $result->num_rows > 0) {
                        while($row = mysqli_fetch_array($result)) {
                    ?>
                    <div class="col-xl-3 col-md-3 col-sm-3">
                        <a class="nav-link" href="pois-subcat?cat=<?php echo $row['insc_id'];?>&name=<?php echo $row['category'];?>">
